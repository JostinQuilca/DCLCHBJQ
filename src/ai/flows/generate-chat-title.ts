 ...
 *
 * - generateChatTitle - A function that generates a chat title from the first message.
...
const GenerateChatTitleOutputSchema = z.object({
  title: z.string().describe('A short, concise title for the chat conversation (3-5 words max).'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `Genera un título corto y conciso (máximo 3-5 palabras) en español para una conversación de chat que comienza con este mensaje:

"{{{message}}}"

El título debe resumir el tema principal. No incluyas comillas en el título.`,
});
